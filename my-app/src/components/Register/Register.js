...
export default function Register() {
  return (
    <div className="register">
      ...
      <input type="text" placeholder="What's your email?"className="input-type"></input>

      <span className="font-14">Create a password</span>
      <input type="text" placeholder="Create a password" className="input-type"></input>
      <span className="font-14">What should we call you?</span>
      <input type="text" placeholder="Enter a profile name" className="input-type"></input>
      ...
      <div className="d-flex flex-wrap justify-content-between">
        <div>
          ...
          <label for="male" className="font-12">Male</label>
        </div>
        <div>
          <input type="radio" id="female" value="female" />
          <label for="female" className="font-12">Female</label>
        </div>
        <div>
          <input type="radio" id="non" value="non" />
          <label for="non" className="font-12">Non-Binary</label>
        </div>
        <div>
          <input type="radio" id="others" value="others" />
          <label for="others" className="font-12">Others</label>
        </div>
        <div>
          <input type="radio" id="not" value="not" />
          <label for="not" className="font-12">Prefer not to say</label>
        </div>
      </div>
      <div className="d-flex gap-2 pt-2 pb-3 fw-semi tick-box ">
        <input type="checkbox" id="reg" name="reg" value="reg" className="accent" />
      ...
      <p className="font-10">
        ...
        <a ><span className="color-green">Terms and Conditions of Use </span><span>.</span> </a></p>
      <p className="font-10">
        To learn more about how Spotify collects, uses, shares and protects your
        personal data,
        <a ><span className="color-green"> please see Spotify's Privacy Policy</span><span>.</span></a></p>

      <button type="button" className="register-signup">Sign up</button>

      <p className="text-center">Have an account?<a ><span className="color-green">Log in</span><span>.</span></a></p>
    </div>
  );
}
